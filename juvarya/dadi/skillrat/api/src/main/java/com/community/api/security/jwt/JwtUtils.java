...
public class JwtUtils {
	...
	public String generateJwtToken(LoggedInUser loggedInUserDetails) throws JsonProcessingException {
		...
		return Jwts.builder().setSubject(json).addClaims(Map.of(ROLES, loggedInUserDetails.getRoles()))
				.setIssuedAt(new Date()).setExpiration(new Date((new Date()).getTime() + jwtExpirationMs))
				.signWith(SignatureAlgorithm.HS512, jwtSecret).compact();
	}

	private long getExpiration(int expiryInDays) {
		return new Date().toInstant().plus(expiryInDays, ChronoUnit.DAYS).toEpochMilli();
	}
